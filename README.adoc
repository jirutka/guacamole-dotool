...
If you want to build it from source instead (highly recommended), set environment variable `npm_config_build_from_source=true` when running `npm install`.
This tells prebuild-install (used by canvas) to compile the native module from source instead of downloading a prebuilt binary.
